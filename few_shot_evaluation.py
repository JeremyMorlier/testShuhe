...
class EpisodicGenerator:
    def __init__(
        ...
        assert datasetName != None or num_elements_per_class != None, (
            "datasetName and num_elements_per_class can't both be None"
        )
    ...
    def get_query_size(self, choice_classes, n_queries):
        if n_queries == 0:
            ...
            query_size = min(
                [int(0.5 * self.num_elements_per_class[c]) for c in choice_classes]
            )
    ...
    def get_support_size(self, choice_classes, query_size, n_shots):
        ...
        if n_shots == 0:
            ...
            support_size = sum(
                [
                    math.ceil(
                        beta * min(100, self.num_elements_per_class[c] - query_size)
                    )
    ...
    def get_number_of_shots(self, choice_classes, support_size, query_size, n_shots):
        if n_shots == 0:
            ...
            alphas = torch.Tensor(
                np.random.rand(n_ways) * (np.log(2) - np.log(0.5)) + np.log(0.5)
            )  # sample uniformly between log(0.5) and log(2)
            proportions = torch.exp(alphas) * torch.cat(
                [torch.Tensor([self.num_elements_per_class[c]]) for c in choice_classes]
            )
            proportions /= proportions.sum()  # make sum to 1
            n_shots_per_class = (
                (proportions * (support_size - n_ways)).int() + 1
            ).tolist()
            n_shots_per_class = [
                min(n_shots_per_class[i], self.num_elements_per_class[c] - query_size)
    ...
    def get_number_of_queries(self, choice_classes, query_size, unbalanced_queries):
        if unbalanced_queries:
            ...
            while (
                prob_dist.min() * query_size * len(choice_classes) < 1
            ):  # if there is a class with less than one query resample
                prob_dist = np.random.dirichlet(alpha)
            n_queries_per_class = self.convert_prob_to_samples(
                prob=prob_dist, q_shot=query_size * len(choice_classes)
            )
        else:
    ...
    def sample_indices(
        self, num_elements_per_chosen_classes, n_shots_per_class, n_queries_per_class
    ):
        shots_idx = []
        queries_idx = []
        for k, q, elements_per_class in zip(
            n_shots_per_class, n_queries_per_class, num_elements_per_chosen_classes
        ):
    ...
    def sample_episode(
        self, ways=0, n_shots=0, n_queries=0, unbalanced_queries=False, verbose=False
    ):
        """
        Sample an episode
        """
        ...
        n_shots_per_class = self.get_number_of_shots(
            choice_classes, support_size, query_size, n_shots
        )
        n_queries_per_class = self.get_number_of_queries(
            choice_classes, query_size, unbalanced_queries
        )
        shots_idx, queries_idx = self.sample_indices(
            [self.num_elements_per_class[c] for c in choice_classes],
            n_shots_per_class,
            n_queries_per_class,
        )
        ...
        if verbose:
            ...
            print(
                f"n_ways={len(choice_classes)}, q={query_size}, S={support_size}, n_shots_per_class={n_shots_per_class}"
            )
    ...
    def get_features_from_indices(self, features, episode, validation=False):
        ...
        """
        choice_classes, shots_idx, queries_idx = (
            episode["choice_classes"],
            episode["shots_idx"],
            episode["queries_idx"],
        )
        if validation:
...
class ImageNetGenerator(EpisodicGenerator):
    ...
    def __init__(self, **kwargs):
        ...
        # Read ImageNet graph
        split = {"train": "TRAIN", "test": "TEST", "validation": "VALID"}[
            self.datasetName.split("_")[-1]
        ]
        with open(
            os.path.join(
                get_repository_path(), "datasets", "ilsvrc_2012_dataset_spec.json"
            ),
            "r",
        ) as file:
        ...
        self.node_candidates = [
            ...
            if 5 <= len(self.get_spanning_leaves(node)) <= 392
        ]
...
class OmniglotGenerator(EpisodicGenerator):
    ...
    def select_classes(self, ways):
        superclass_id = str(
            torch.randint(self.dataset["num_superclasses"], (1, 1)).reshape(-1).item()
        )
        classes_ids = self.dataset["classes_per_superclass"][superclass_id]
        num_sampled_classes = (
            ways
            if ways != 0
            else torch.randint(5, min(len(classes_ids), 50), (1, 1)).reshape(-1)
        )

        return torch.tensor(classes_ids)[
            torch.randperm(len(classes_ids))[:num_sampled_classes].tolist()
        ]
...
class MetaAlbumsGenerator(EpisodicGenerator):
    ...
    def select_classes(self, ways, album=None):
        ...
        if self.metaalbum:
            ...
            num_elements_per_class = self.dataset[
                f"metaalbum_{album}_{self.datasetName.split('_')[-1]}"
    ...
    def sample_episode(
        ...
        shots_idx, queries_idx = self.sample_indices(
            [num_elements_per_class[c] for c in choice_classes],
            n_shots_per_class,
            n_queries_per_class,
        )
        ...
        if verbose:
            ...
            print(f"chosen class: {choice_classes}")
            print(
                f"n_ways={len(choice_classes)}, q={query_size}, n_shots_per_class={n_shots_per_class}"
            )
            print(f"queries per class:{n_queries_per_class}")
            print(f"shots_idx: {shots_idx}")
            print(f"queries_idx: {queries_idx}")
    ...
    def get_features_from_indices(self, features, episode, validation=False):
        ...
        Get features from a list of all features and from a dictonnary describing an episode
        """
        album, choice_classes, shots_idx, queries_idx = (
            episode["album"],
            episode["choice_classes"],
            episode["shots_idx"],
            episode["queries_idx"],
...
if __name__ == "__main__":
    ...
    for _ in range(1):
        ...
        Generator = {
            "metadataset_omniglot": OmniglotGenerator,
            "metadataset_imagenet": ImageNetGenerator,
        }.get(
            args.dataset.replace("_train", "")
            .replace("_test", "")
            .replace("_validation", "")
        ...
        num_elements_per_class = (
            [len(feat["features"]) for feat in feature]
            if args.test_features != ""
            else None
        )
        generator = Generator(
            datasetName=args.dataset + "_test",
            dataset_path=args.dataset_path,
            num_elements_per_class=num_elements_per_class,
        )
        episode = generator.sample_episode(
            n_queries=args.few_shot_queries,
            ways=args.few_shot_ways,
            n_shots=args.few_shot_shots,
            unbalanced_queries=args.few_shot_unbalanced_queries,
            verbose=True,
        )
        if args.test_features != "":
            ...
            print(
                "acc={:.2f}% (±{:.2f}%)".format(torch.mean(accs).item(), (up - low) / 2)
            )
